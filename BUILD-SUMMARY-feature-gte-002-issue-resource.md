...
The Issue resource — the main unit of work in the task ledger. Replaces what bd called a "task" in the Go implementation.

### Files added/changed

```
apps/arbiter/lib/arbiter/tasks.ex                                   (M) register Issue + Issue.Version
apps/arbiter/lib/arbiter/tasks/workspace.ex                         (M) added :prefix attribute (default "bd", lowercase alnum, max 16)
apps/arbiter/lib/arbiter/tasks/issue.ex                             (+) main resource
apps/arbiter/lib/arbiter/tasks/issue/changes/generate_id.ex         (+) "{prefix}-{6 char base36}" PK generation
apps/arbiter/lib/arbiter/tasks/issue/changes/inherit_tracker_type.ex (+) inherit from workspace.config when not explicit
apps/arbiter/lib/arbiter/tasks/issue/changes/guard_status.ex        (+) FSM enforcement
...
apps/arbiter/test/arbiter/tasks/issue_test.exs                      (+) 23 tests
```

## Acceptance check (from task gte-002)
...
| `tracker_type` defaults to `:none`, can be overridden per-task | ✅ tested both inheritance (workspace tracker→issue) and explicit override |
...
| Audit trail captures actor | ✅ paper_trail config has `store_action_name? true` + `store_action_inputs? true`; tests assert action_name on version rows. (Actor — `:created_by` etc. — lands when we add auth in a later task.) |
...
1. **`Dependency` resource (task-to-task edges)** — gte-003's scope.
...
5. **Actor tracking on paper_trail** — needs an auth task first (not in current plan; could fold into LiveView when needed).
...
- **`Issue.Version` resource is auto-generated by AshPaperTrail.** Lives in the `Arbiter.Tasks.Issue.Version` module (generated, not in source). Listed in domain resources. Useful but invisible — worth documenting elsewhere that "if you see Issue.Version queries, it's the audit log."
...
mix test apps/arbiter/test/arbiter/tasks/issue_test.exs
...
# Spot check from iex
iex -S mix
> alias Arbiter.Tasks.{Workspace, Issue}
...
> Ash.read!(Arbiter.Tasks.Issue.Version)  # → 2 rows: create + close
```
