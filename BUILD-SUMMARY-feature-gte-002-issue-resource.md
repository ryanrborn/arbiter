...
```
apps/gt_elixir/lib/gt_elixir/beads.ex                                   (M) register Issue + Issue.Version
apps/gt_elixir/lib/gt_elixir/beads/workspace.ex                         (M) added :prefix attribute (default "bd", lowercase alnum, max 16)
apps/gt_elixir/lib/gt_elixir/beads/issue.ex                             (+) main resource
apps/gt_elixir/lib/gt_elixir/beads/issue/changes/generate_id.ex         (+) "{prefix}-{6 char base36}" PK generation
apps/gt_elixir/lib/gt_elixir/beads/issue/changes/inherit_tracker_type.ex (+) inherit from workspace.config when not explicit
apps/gt_elixir/lib/gt_elixir/beads/issue/changes/guard_status.ex        (+) FSM enforcement
apps/gt_elixir/priv/repo/migrations/20260519194239_add_issue_resource.exs (+) creates issues + issues_versions
apps/gt_elixir/priv/resource_snapshots/repo/issues/...                  (+) Ash snapshot
apps/gt_elixir/priv/resource_snapshots/repo/issues_versions/...         (+) AshPaperTrail snapshot
apps/gt_elixir/test/gt_elixir/beads/issue_test.exs                      (+) 23 tests
```
...
- **`Issue.Version` resource is auto-generated by AshPaperTrail.** Lives in the `GtElixir.Beads.Issue.Version` module (generated, not in source). Listed in domain resources. Useful but invisible — worth documenting elsewhere that "if you see Issue.Version queries, it's the audit log."
...
mix test apps/gt_elixir/test/gt_elixir/beads/issue_test.exs
# Expect: 23 tests, 0 failures

# Full suite
mix test
# Expect: 36 gt_elixir + 5 gt_elixir_web + 1+1 gt_elixir_cli = 43 tests, 0 failures

# Spot check from iex
iex -S mix
> alias GtElixir.Beads.{Workspace, Issue}
...
> Ash.read!(GtElixir.Beads.Issue.Version)  # → 2 rows: create + close
```
