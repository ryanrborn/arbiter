...
`Arbiter.Trackers.Tracker` behaviour from gte-019. Ships:

- `Arbiter.Trackers.Jira` — adapter (`fetch/1`, `transition/2`,
  `update_fields/2`, `link_for/1`, `parse_ref/1`, `list_transitions/1`).
- `Arbiter.Trackers.Jira.ADF` — Markdown → ADF converter for rich-text
  fields (description, qa_notes, deployment_notes).
- `Arbiter.Trackers.Jira.Config` — workspace-scoped config resolver
  (host / project_key / email / token / status_map / field_ids), backed by
  per-process state with an Application-env fallback.
- `Arbiter.Trackers.Jira.Error` — normalised error struct with kinds
  mirroring `Arbiter.GitHub.Error` plus two Jira-specific kinds:
  `:transition_not_found` and `:config_missing`.

`Arbiter.Trackers.@adapters` now maps `:jira → Jira`. The pre-existing
`trackers_test.exs` was updated; `Trackers.for_type(:jira)` resolves cleanly.

## Files

- `apps/arbiter/lib/arbiter/trackers/jira.ex` — main adapter (~330 LOC).
- `apps/arbiter/lib/arbiter/trackers/jira/adf.ex` — Markdown → ADF.
- `apps/arbiter/lib/arbiter/trackers/jira/config.ex` — config resolver.
- `apps/arbiter/lib/arbiter/trackers/jira/error.ex` — Error struct.
- `apps/arbiter/lib/arbiter/trackers.ex` — added `:jira → Jira`.
- `apps/arbiter/test/arbiter/trackers/jira_test.exs` — 22 tests for
  adapter callbacks (Req.Test stubs only — never hits a real endpoint).
- `apps/arbiter/test/arbiter/trackers/jira/adf_test.exs` — 19 tests for
  the Markdown → ADF converter.
- `apps/arbiter/test/arbiter/trackers_test.exs` — updated for new
  registry shape.
- `config/test.exs` — added `:arbiter, :jira_http_stub, true`.
...
`Application.get_env(:arbiter, :jira_default_config)` is consulted next.
...
`:arbiter, :jira_http_stub, true` in `config/test.exs`. Pattern is
identical to the existing `Arbiter.GitHub` test wiring. No network calls
...
- Rate-limit observability (`:persistent_term` cache like
  `Arbiter.GitHub.rate_limit/0`) for Jira. Jira's headers differ
