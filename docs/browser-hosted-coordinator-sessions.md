...
2. **Idle deadline (`Arbiter.Sessions.IdleReaper`).** A periodic sweep
   ...
   marked is never a candidate, however stale its clock reads. `last_turn_at`
   is a new column alongside the existing `last_client_at`: a turn actually
   running is activity distinct from a client merely being attached, and
   without it a long-running unattended agent turn would look idle to the
   sweep.
