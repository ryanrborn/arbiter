...
| Site | Today | Becomes |
|---|---|---|
| ReviewGate clean approve | `apps/arbiter/lib/arbiter/worker/review_gate.ex:3743` (`stamp_reviewed_head`) writes `last_reviewed_sha` | `Coverage.record(kind: :reviewed, round: state.round, net_diff_id: …)` — **and the write is no longer best-effort**: a failed write must page, because a silently-missing row *is* the #1585 stall |
...
modelled on `apps/arbiter/lib/arbiter/worker/review_gate.ex:2848` (`verdict_guards`), which already proves the pattern works for four guards:
...
| G16's escalation shapes | `apps/arbiter/lib/arbiter/worker/review_gate.ex:2159` (`escalate_commit_gate`) | one fingerprint predicate, one escalation, two remediation strings |
