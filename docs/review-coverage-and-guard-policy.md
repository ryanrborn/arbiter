...
| G13 | Shared guard dispatcher + terminal handling | `apps/arbiter/lib/arbiter/worker/review_gate.ex:2829` (`run_verdict_guard`), `apps/arbiter/lib/arbiter/worker/review_gate.ex:2867` (`fail_closed`), registry `apps/arbiter/lib/arbiter/worker/review_gate.ex:2814` (`verdict_guards`) | Four guards drifting apart | — | — | — |
...
modelled on `apps/arbiter/lib/arbiter/worker/review_gate.ex:2814` (`verdict_guards`), which already proves the pattern works for four guards:
