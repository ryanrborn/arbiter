...
latch (`apps/arbiter/lib/arbiter/worker/watchdog.ex:4534` (`clear_reviewed_latch`), `apps/arbiter/lib/arbiter/workflows/merge_queue.ex:1701` (`clear_reviewed_latch`)), and the Watchdog invents a *memo invalidation* on top
of the suspension (`apps/arbiter/lib/arbiter/worker/watchdog.ex:4419` (`load_recorded_reviewed_sha`)). All of that machinery is an attempt to
...
| W6 | Unreviewed head → back to review, else page once | `apps/arbiter/lib/arbiter/worker/watchdog.ex:4346` (`resolve_stale_reviewed_head`) | bd-6bg54c: the 303-retry loop | **Fails the worker** with `{:unreviewed_head, head}` and buys a review round — since P7 scoped by the ReviewGate to the delta since the last covered commit when one is an ancestor (§4.5), and routed through W13/W14's resume path so a fix pass still holding the family defers instead of paging; when the resume budget is spent, pages and stops | `Worker.fail` + resume, or one escalation | 2 |
| W7 | Forge atomic precondition (`expected_sha`) | `apps/arbiter/lib/arbiter/worker/watchdog.ex:2081` (`apply_guarded_merge`) | The residual poll→merge window | A racing push turns into a merge failure | **Unbounded retrying.** Retries the merge call every poll; `apps/arbiter/lib/arbiter/worker/watchdog.ex:276` (`default_merge_fail_notify_threshold`) gates only the *page*, after which **`max_polls: :infinity`** and a re-page every cadence — no terminal state | 2 |
| W8 | Latch suspension for fleet-authored pushes (update-branch only, since P7) | `apps/arbiter/lib/arbiter/worker/watchdog.ex:4534` (`clear_reviewed_latch`) | Deadlocking the fleet's own rebase/fix-pass against its own guard | The guard is **deliberately** scoped to advances the fleet did not initiate. The consequence is that the CI `fix_pass` path (`apps/arbiter/lib/arbiter/worker/watchdog.ex:2592` (`clear_reviewed_latch`)) re-latches to the fix-pass head and merges content no reviewer saw — a deliberate scoping choice, but the same shape #1498 exists to stop. §4.5 argues it should change. **P7 (bd-60r6wp / #1738) changed it:** only update-branch still suspends; a fix pass or conflict resolver keeps the approved baseline (`note_authored_push/1`) and its head is judged on content by W5/W6 | Baseline floats to the new head | 2 |
| W9 | Baseline tracking per poll | `apps/arbiter/lib/arbiter/worker/watchdog.ex:4445` (`track_reviewed_baseline`) | Losing the baseline across polls | Re-pins to a stale head while suspended | — | 2 |
| W10 | `via_review_gate` outcome pinning | `apps/arbiter/lib/arbiter/worker/watchdog.ex:1493` (`effective_outcome`) | A gate-approved lane whose forge shows no approval | Approval never lapses ⇒ W3's memo never invalidates (the bd-6bg54c cause-B mechanism) | — | 1 |
...
| # | Guard | Anchor | Protects against | Misfire mode | On failure | Patches |
|---|---|---|---|---|---|---|
| C1 | bd-ofql8k commit gate (`:uncommitted` / `:no_commits` / `:secret_in_commit`) | `apps/arbiter/lib/arbiter/worker.ex:3866` (`commit_gate`) | A worker printing `arb done` over uncommitted or absent work; committed agent-config bearer tokens | Non-branch worktrees would false-positive, hence the branch check; git errors | **Fails open** on git error; otherwise diverts to a nudge relaunch | 3 |
| C2 | Rejection parking | `apps/arbiter/lib/arbiter/worker.ex:5718` (`park_rejected`) | — | Since P9, `park_rejected/4` takes a park reason: with one it writes `Run.status = :review_parked` and pages once; without one (a genuine REQUEST_CHANGES only) it is the pre-P9 `Run.status = :failed` via `apps/arbiter/lib/arbiter/worker.ex:5778` (`fail_reason_for`) | `fail_now` | 2 |
| C3 | Fix-round budget and non-convergence digest | `apps/arbiter/lib/arbiter/worker.ex:5802` (`maybe_dispatch_fix_round`) | bd-a9zb7w: a rejection nobody scheduled an implementer for | Identical-findings digest stops the loop — the one guard already shaped the way §5 wants | One escalation | 2 |
...
* **Two guards retry without any bound: M3 and W7.** M3 leaves the queue item's
  ...
  *paging* is bounded (`apps/arbiter/lib/arbiter/worker/watchdog.ex:276`
  ...
  (`apps/arbiter/lib/arbiter/worker/watchdog.ex:2118` (`merge_fail_count`)) and
  re-issues the merge call on the next poll, and the paging branch lifts
  `apps/arbiter/lib/arbiter/worker/watchdog.ex:2160` (`max_polls`) to `:infinity`
...
| Site | Today | Becomes |
|---|---|---|
| ReviewGate clean approve | `apps/arbiter/lib/arbiter/worker/review_gate.ex:3923` (`stamp_reviewed_head`) writes `last_reviewed_sha` | `Coverage.record(kind: :reviewed, round: state.round, net_diff_id: …)` — **and the write is no longer best-effort**: a failed write must page, because a silently-missing row *is* the #1585 stall |
...
| Watchdog fleet push (update-branch / rebase) | `apps/arbiter/lib/arbiter/worker/watchdog.ex:4534` (`clear_reviewed_latch`) suspends the guard | `Coverage.record(kind: :mechanical, …)` **only if** the fingerprint matches; otherwise nothing is recorded and the new head is honestly uncovered |
| Watchdog CI `fix_pass` | `apps/arbiter/lib/arbiter/worker/watchdog.ex:2592` (`clear_reviewed_latch`) — merges unguarded | nothing. A `fix_pass` changes content by construction, so its head is `:uncovered` and routes to a scoped re-review. **This closes the hole in §2.6.** **P7 ✅ (bd-60r6wp / #1738)** |
...
* **Today:** `apps/arbiter/lib/arbiter/worker/watchdog.ex:2592` (`clear_reviewed_latch`) **suspends** the guard for the fleet's own push, then
...
modelled on `apps/arbiter/lib/arbiter/worker/review_gate.ex:3059` (`verdict_guards`), which already proves the pattern works for four guards:
...
| Watchdog latch/suspension/memo (`reviewed_sha`, `recorded_reviewed_sha`, `recorded_sha_loaded?`, `cleared_recorded_sha`, `latch_suspended_at_head`, `head_lag_polls`) | `apps/arbiter/lib/arbiter/worker/watchdog.ex:4445` (`track_reviewed_baseline`) | coverage rows |
...
| G16's escalation shapes | `apps/arbiter/lib/arbiter/worker/review_gate.ex:2326` (`escalate_commit_gate`) | one fingerprint predicate, one escalation, two remediation strings |
...
`apps/arbiter/lib/arbiter/worker/watchdog.ex:4052` (`observe_coverage`) and
...
* **Webhook-driven merge detection.** Still the right upgrade
  (`apps/arbiter/lib/arbiter/worker/watchdog.ex:1493` (`effective_outcome`)
