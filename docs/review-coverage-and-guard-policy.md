...
|---|---|---|---|---|---|---|
| G1 | Pre-spawn commit check — branch has commits ahead of target | `apps/arbiter/lib/arbiter/worker/review_gate.ex:3057` (`reviewer_commit_check`) | bd-1mksks: reviewing an empty branch, reviewer reports "no work" | Git hiccup reads as "no commits" | **Fails open** (git errors → proceed); genuine `{:ok, false}` → `escalate_pre_review` | 2 (bd-1mksks, bd-ofql8k) |
| G2 | Empty diff-range guard (`base_sha == head_sha`) | `apps/arbiter/lib/arbiter/worker/review_gate.ex:3101` (`empty_diff_guard`) | bd-31bh37: target already absorbed the commits; bogus REQUEST_CHANGES | A legitimately-absorbed branch escalates instead of completing | Escalates via `apps/arbiter/lib/arbiter/worker/review_gate.ex:2644` (`escalate_pre_review`) — since P9, **parks** `:empty_diff`, one escalation, run recorded `review_parked` | 1 |
...
| G4 | Timeout → `:no_verdict`, not `:request_changes` | `apps/arbiter/lib/arbiter/worker/review_gate.ex:2678` (`escalate_timeout`) | bd-216r3e: synthetic single-finding REQUEST_CHANGES → self-sustaining re-dispatch loop | — (this one is a *fix* to a prior misfire) | Records `verdict: :timed_out`, reports `:no_verdict` | 1 |
...
| G13 | Shared guard dispatcher + terminal handling | `apps/arbiter/lib/arbiter/worker/review_gate.ex:2306` (`run_verdict_guard`), `apps/arbiter/lib/arbiter/worker/review_gate.ex:2344` (`fail_closed`), registry `apps/arbiter/lib/arbiter/worker/review_gate.ex:2291` (`verdict_guards`) | Four guards drifting apart | — | — | — |
...
| Site | Today | Becomes |
|---|---|---|
| ReviewGate clean approve | `apps/arbiter/lib/arbiter/worker/review_gate.ex:3172` (`stamp_reviewed_head`) writes `last_reviewed_sha` | `Coverage.record(kind: :reviewed, round: state.round, net_diff_id: …)` — **and the write is no longer best-effort**: a failed write must page, because a silently-missing row *is* the #1585 stall |
...
modelled on `apps/arbiter/lib/arbiter/worker/review_gate.ex:2291` (`verdict_guards`), which already proves the pattern works for four guards:
