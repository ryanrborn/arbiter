defmodule Arbiter.Tasks.Issue.Calcs do
  @moduledoc """
  Batch calculations for the parent-with-progress rollup.

  A task is a *parent* of every task it points at with a `:parent_of`
  dependency edge (`from_issue` is the parent, `to_issue` the child — see
  `Arbiter.Tasks.Dependency`). These calculations roll the children up into a
  ...
  grouping is now just a normal task with `:parent_of` children. Like the old
