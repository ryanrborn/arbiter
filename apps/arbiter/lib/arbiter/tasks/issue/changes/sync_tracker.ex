    ...
    case Trackers.fetch(issue) do
      {:ok, raw} ->
        if upstream_closed?(issue.tracker_type, raw) do
          ...
          Logger.warning(
            "SyncTracker: upstream still open after close transition for task=#{issue.id} " <>
          ...
          case Trackers.transition(issue, :closed) do
            ...
            {:error, reason} ->
              Logger.warning(
                "SyncTracker: follow-up close also failed for task=#{issue.id} " <>
                  "tracker=#{issue.tracker_type} ref=#{issue.tracker_ref}: #{inspect(reason)}"
              )
          end
        end
      ...
      {:error, reason} ->
        Logger.warning(
          "SyncTracker: could not verify closed state for task=#{issue.id} " <>
            "tracker=#{issue.tracker_type} ref=#{issue.tracker_ref}: #{inspect(reason)} " <>
