defmodule Arbiter.Tasks.Issue.Changes.CreateUpstream do
  @moduledoc """
  After-transaction hook for `Issue.create`: when the task has a tracker
  ...
  Runs in an `after_transaction` hook (not `after_action`) so the task is
  **committed before** the upstream call. That preserves the task even when
  ...
  re-link the task without orphaning anything.

  On a successful upstream create the returned ref is persisted to the task
  ...
  per-process slot returned by `last_error/0` — the task is durable, the
  action result is success, and the controller picks the stash up after
  `Ash.create` returns and renders the 502 + task body that lets the CLI
  ...
  which mangles the shape into a string and discards the kind/task_id
  ...
  defp do_create(issue) do
    case load_workspace(issue.workspace_id) do
      nil ->
        # Task was created without a resolvable workspace — can't dispatch.
        Logger.warning("CreateUpstream: task=#{issue.id} has no resolvable workspace; skipping")
      ...
      workspace ->
        ...
        case Trackers.create_for_workspace(workspace, attrs) do
          ...
          {:error, :not_supported} ->
            ...
            # stub adapter). Local-only task; not a failure.
            {:ok, issue}

          {:error, reason} ->
            Logger.warning(
              "CreateUpstream: failed to create upstream for task=#{issue.id} " <>
            ...
            # Return the task so the action result is `{:ok, issue}` — the
  ...
  defp persist_ref(issue, ref) do
    case Ash.update(issue, %{tracker_ref: ref}, action: :update) do
      ...
      {:error, reason} ->
        ...
        Logger.error(
          "CreateUpstream: created upstream ref=#{ref} for task=#{issue.id} but " <>
  ...
  defp upstream_error_message(issue, %{message: msg}) when is_binary(msg) do
    "task #{issue.id} created locally but upstream #{issue.tracker_type} create failed: #{msg}"
  end

  defp upstream_error_message(issue, reason) do
    "task #{issue.id} created locally but upstream #{issue.tracker_type} create failed: " <>
      inspect(reason)
  end
end
