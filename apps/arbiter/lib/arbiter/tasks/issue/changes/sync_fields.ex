defmodule Arbiter.Tasks.Issue.Changes.SyncFields do
  @moduledoc """
  ...
  Mirrors the teardown pattern in `Arbiter.Tasks.Issue.Changes.SyncTracker`.
  """
      ...
      {:error, reason} ->
        Logger.warning(
          "SyncFields: failed to sync fields #{inspect(Map.keys(changed))} " <>
            "for task=#{issue.id} tracker=#{issue.tracker_type} " <>
            "ref=#{issue.tracker_ref}: #{inspect(reason)}"
        )
    end
  rescue
    e ->
      Logger.warning("SyncFields: error syncing task=#{issue.id}: #{Exception.message(e)}")
  catch
    :exit, reason ->
      Logger.warning("SyncFields: exit syncing task=#{issue.id}: #{inspect(reason)}")
  end
