defmodule Arbiter.Mergers do
  ...
  Mirrors `Arbiter.Trackers`. The `Direct` and `:github` adapters ship now;
  `:gitlab` is added in a later directive.
  ...
  alias Arbiter.Mergers.{Direct, Github}
  ...
  @adapters %{
    direct: Direct,
    github: Github
    # :gitlab wired up in a later directive
  }
