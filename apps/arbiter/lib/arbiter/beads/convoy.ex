defmodule Arbiter.Beads.Convoy do
  @moduledoc """
  ...
  * `closed_issues` — count of tracked issues with status :closed
  ...
  actions do
    ...
    create :create do
      ...
      change {Arbiter.Beads.Convoy.Changes.GenerateId, []}
    end
    ...
    update :close do
      ...
      change {Arbiter.Beads.Convoy.Changes.SetClosedReason, []}
    end
  end
  ...
  relationships do
    ...
    has_many :memberships, Arbiter.Beads.ConvoyMembership do
      destination_attribute :convoy_id
      public? true
    end

    many_to_many :issues, Arbiter.Beads.Issue do
