defmodule Arbiter.Workflow do
  ...
  Then:
      ...
      iex> Arbiter.Workflow.run(Examples.Boring, %{name: "Ryan"})
  ...
  `use Arbiter.Workflow` accepts `extends:`, `expansions:`, and `aspects:`
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      import Arbiter.Workflow, only: [step: 2]

      @before_compile Arbiter.Workflow
    end
  end

  @doc """
  Declares a step's metadata. Must appear inside a `use Arbiter.Workflow`
  module.
  ...
  defmacro __before_compile__(env) do
    ...
    if missing != [] do
      raise CompileError,
        description:
          "Arbiter.Workflow #{inspect(module)}: steps declared in `use` but " <>
    ...
    if orphans != [] do
      raise CompileError,
        description:
          "Arbiter.Workflow #{inspect(module)}: `step :name, ...` defined for " <>
    ...
    Enum.each(declared, fn {name, opts} ->
      ...
      if unknown != [] do
        raise CompileError,
          description:
            "Arbiter.Workflow #{inspect(module)}: step #{inspect(name)} declares " <>
