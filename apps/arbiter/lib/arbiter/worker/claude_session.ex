defmodule Arbiter.Worker.ClaudeSession do
  ...
  *results* are displayed but never trip completion, so an worker that greps
  ...
  def start(opts) when is_list(opts) do
    with {:ok, owner} <- fetch_owner(opts),
      ...
      session_config = %{
        ...
        # Secret-flagged worker env var values to scrub from all output surfaces
        # (bd-62d3jh). Resolved once here from the task's workspace and threaded
        # to the emit path (`redact_line/2`).
        redact_values: Arbiter.Worker.WorkerEnv.secret_values(task_id)
      }
  ...
  defp env_pairs(opts, task_id) do
    ...
    # User-defined worker env vars for the task's workspace (bd-62d3jh). Sit
    # after the release/dev-server cleanups but before caller-explicit `:env`
    # (agent auth) and the always-last ARB_ACOLYTE_BEAD_ID guard, so a user var
    # can never clobber the agent's auth or the self-recursion guard.
    worker_env = Arbiter.Worker.WorkerEnv.pairs(task_id)
