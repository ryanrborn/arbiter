defmodule Arbiter.Agents.Routing.Policy do
  @moduledoc """
  Behaviour for the function `(bead, workspace, ledger_snapshot) → agent
  choice`.

  A routing policy decides which agent adapter + config to use for a given
  bead at dispatch time. It is intentionally a *thin* layer above
  `Arbiter.Agents` (the dispatcher): the dispatcher resolves the workspace
  default; the policy can override it per-bead based on priority, budget,
  or any other signal.
  ...
  @callback choose(bead :: Issue.t(), workspace :: Workspace.t() | nil, ledger_snapshot) ::
              choice
end
