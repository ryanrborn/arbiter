defmodule Arbiter.GitHub do
  ...
  ## Design
    ...
    * Every public function returns `{:ok, value}` or
      `{:error, %Arbiter.GitHub.Error{}}`. The `Error.kind` is a small enum
      mapped from HTTP status (see `t:Arbiter.GitHub.Error.kind/0`).
    ...
    * Every response — successful or 4xx/5xx — updates a process-global
      ...
      `:arbiter_github_rate_limit`. Read it with `rate_limit/0`. This avoids
  ...
  In `:test`, application env `:arbiter, :github_http_stub, true` causes
  this module to inject `plug: {Req.Test, Arbiter.GitHub.HTTP}` into every
  ...
  @rate_limit_key :arbiter_github_rate_limit
  ...
  `Arbiter.Workflows.MergeQueue` to decide when to spawn the conflict-
  resolution worker.
  ...
  defp fetch_token!(opts) do
    case Keyword.get(opts, :token) || System.get_env("GITHUB_TOKEN") do
      ...
      _ ->
        raise ArgumentError,
              "Arbiter.GitHub: no token supplied (set GITHUB_TOKEN env var " <>
                "or pass opts[:token])"
    end
  end
  ...
  defp stub_opts(opts) do
    cond do
      ...
      Application.get_env(:arbiter, :github_http_stub, false) ->
