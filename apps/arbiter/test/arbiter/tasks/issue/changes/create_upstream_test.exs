defmodule Arbiter.Tasks.Issue.Changes.CreateUpstreamTest do
  @moduledoc """
  ...
  (`:github_http_stub` is true in the test env). Asserts that:
    ...
    * an upstream failure leaves the task intact and surfaces a
  ...
  describe "create on a github-configured workspace" do
    test "POSTs to /repos/:owner/:repo/issues and persists the returned number as tracker_ref" do
      ...
      # And the persisted task reflects the ref (no stale in-memory shape).
  ...
  describe "upstream-failure semantics" do
    test "GitHub 500 leaves the task intact and stashes :upstream_create_failed" do
      stub(fn conn ->
        conn
      ...
      # The action succeeds — task is durable. The upstream error is stashed
      ...
      assert %{
               ...
               tracker_type: :github,
               message: msg
             } = Arbiter.Tasks.Issue.Changes.CreateUpstream.last_error()

      assert task_id == issue.id
      assert msg =~ "upstream github create failed"

      # And `last_error/0` drains, so a follow-up call returns nil.
      assert is_nil(Arbiter.Tasks.Issue.Changes.CreateUpstream.last_error())

      # Task must exist — local create succeeded; only the upstream side failed.
      assert {:ok, persisted} = Ash.get(Issue, task_id)
      assert persisted.title == "doomed"
      assert is_nil(persisted.tracker_ref)
    end

    test "missing GitHub token stashes :upstream_create_failed but task is durable" do
      ws = github_workspace()
      System.delete_env(@env_var)

      assert {:ok, issue} = Ash.create(Issue, %{title: "no token", workspace_id: ws.id})

      assert %{kind: :upstream_create_failed, task_id: task_id} =
               Arbiter.Tasks.Issue.Changes.CreateUpstream.last_error()

      assert task_id == issue.id
      assert {:ok, _persisted} = Ash.get(Issue, task_id)
    end
