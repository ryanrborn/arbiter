defmodule Arbiter.Tasks.DecommissionSweepTest do
  use Arbiter.DataCase, async: false

  alias Arbiter.Tasks.{DecommissionSweep, Issue, Workspace}
  ...
  describe "proposals/0" do
    test "proposes closure for an open HANDOFF task", %{ws: ws} do
      task = new_task(ws, "🤝 HANDOFF: Witness patrol")
    ...
    test "skips closed tasks", %{ws: ws} do
      task = new_task(ws, "🤝 HANDOFF: stale")
      {:ok, _} = Ash.update(task, %{}, action: :close)

      assert [] = DecommissionSweep.proposals() |> Enum.filter(&(&1.task_id == task.id))
    end

    test "keepers list protects gte-026..028 + hq-109 + hq-3be + vs-sy5" do
      # Without actually creating those tasks, just confirm the public
      ...
      proposal_ids = Enum.map(proposals, & &1.task_id)
  ...
  describe "apply!/1" do
    test "closes each proposed task", %{ws: ws} do
      b1 = new_task(ws, "🤝 HANDOFF: a")
      b2 = new_task(ws, "MergeQueue for fakerig - processes merge queue.")

      proposals = DecommissionSweep.proposals()

      proposals_for_test =
        Enum.filter(proposals, &(&1.task_id in [b1.id, b2.id]))
