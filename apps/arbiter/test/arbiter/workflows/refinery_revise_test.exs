defmodule Arbiter.Workflows.RefineryReviseTest do
  @moduledoc """
  ...
  # The Refinery resolves the stub by atom, so we route through :persistent_term
  # keyed on bead id (same trick the conflict-test stub uses): the test seeds a
  # target pid + canned result before driving the Refinery.
  defmodule StubDispatcher do
    @moduledoc false
    @behaviour Arbiter.Workflows.Refinery.ReviseDispatcher
  ...
  defp start_refinery(workspace, opts \\ []) do
    name = :"refinery_revise_#{System.unique_integer([:positive])}"
  ...
  describe "CHANGES_REQUESTED triggers a single revise pass" do
    ...
    test "the same review is never actioned twice across many ticks (debounce)", %{
      ...
      for _ <- 1..5, do: :ok = Refinery.tick(name)
