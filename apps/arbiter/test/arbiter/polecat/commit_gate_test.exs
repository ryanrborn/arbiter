defmodule Arbiter.Polecat.CommitGateTest do
  @moduledoc """
  ...
  defp start_polecat(bead, repo, worktree_path, extra_meta) do
    meta =
      Map.merge(
        %{
  ...
  describe "the commit gate (bd-ofql8k)" do
    test "an arb-done with UNCOMMITTED changes fails + escalates instead of routing to ReviewGate",
      ...
      pid = start_polecat(bead, repo, path, %{commit_nudge_cap: 0})
