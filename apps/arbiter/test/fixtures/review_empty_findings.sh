#!/bin/sh
# Fixture: a reviewer (ReviewGate) worker that emits `VERDICT: REQUEST_CHANGES`
...
# ReviewGate's re-prompt or keeps withholding findings.
#
...
retry="${1:-EMPTY}"
marker="$(git rev-parse --git-common-dir)/review_gate_empty_findings_attempt"
