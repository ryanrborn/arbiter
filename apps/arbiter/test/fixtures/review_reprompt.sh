#!/bin/sh
# Fixture: a reviewer (ReviewGate) worker that emits substantive review output but
# FORGETS the verdict sentinel on its first pass, then supplies one when the
# ReviewGate re-prompts. Exercises the verdict re-prompt path (bd-8v8ays).
#
...
# first pass apart from later ones — the ReviewGate re-runs this same argv, so the
...
marker="$(git rev-parse --git-common-dir)/review_gate_reprompt_attempt"
