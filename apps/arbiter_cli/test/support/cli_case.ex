defmodule ArbiterCli.CliCase do
  @moduledoc """
  ...
  Sets up:
    ...
    * IO capture helpers that also catch `ArbiterCli.Output.Halt`

  Usage:

      use ArbiterCli.CliCase

      test "show prints issue" do
        stub_get("/api/issues/foo-123", %{"id" => "foo-123", "title" => "T"})
        {out, _err, exit_code} = capture(fn ->
          ArbiterCli.Cmd.Show.run(["foo-123"])
        end)
        assert out =~ "foo-123"
        assert exit_code == 0
      end
