defmodule Mix.Tasks.GtElixir.BackfillBeadStatuses do
  @shortdoc "Close beads that have feat() commits on main but are still :open"
  @moduledoc """
  Walk `git log` for `feat(<bead-id>)` commits on a branch and close any
  matching beads still in `:open` / `:in_progress` status.

  This is a recovery tool for the bead-status drift the cutover postmortem
  ...
  ## Usage
      ...
      mix gt_elixir.backfill_bead_statuses              # dry-run (prints proposals)
      mix gt_elixir.backfill_bead_statuses --apply      # actually close beads
      mix gt_elixir.backfill_bead_statuses --branch dev # use a different branch

  ## What you see in dry-run

  A list of beads that would be closed, with the commit SHA and subject
  ...
  alias GtElixir.Beads.StatusBackfill
  ...
  def run(argv) do
    ...
    cond do
      proposals == [] ->
        Mix.shell().info("No drifted beads found. Nothing to do.")
  ...
  defp banner(proposals, :dry_run),
    do: "Would close #{length(proposals)} bead(s):\n"

  defp banner(proposals, :apply),
    do: "Closing #{length(proposals)} bead(s):\n"
