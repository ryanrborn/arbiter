defmodule GtElixir.Workflows.CodeReview do
  @moduledoc """
  ...
  a verdict (`:approve` or `:request_changes`). Two modes:
    ...
    * `:github` — posts inline comments + a top-level review via the
      `GtElixir.GitHub` HTTP client (no `gh` shell-out).
  ...
  ## State
      ...
      %{
        ...
        github_opts: keyword(),      # forwarded to GtElixir.GitHub.* calls
  ...
  `GtElixir.Workflows.CodeReview.Checks`).
  """
  ...
  alias GtElixir.Workflows.CodeReview.{Checks, GithubMode, LocalMode}
