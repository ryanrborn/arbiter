defmodule GtElixir.Workflows.MachineState do
  @moduledoc """
  Persistent state for a single `GtElixir.Workflows.Machine` instance.

  Each row binds a `GtElixir.Workflow` behaviour module to a bead (issue),
  ...
  `workflow_module` is stored as a string (e.g. `"Elixir.GtElixir.Workflow.Example.GreetThenWave"`)
  and reconstituted at load via `Module.safe_concat/1`. The reconstituted
  atom is then validated to ensure it implements the `GtElixir.Workflow`
  behaviour before any of its functions are invoked. See
  `GtElixir.Workflows.Machine.load_workflow_module/1` for the allowlist
  check.
