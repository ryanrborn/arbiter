defmodule GtElixir.Workflows.Machine do
  @moduledoc """
  Workflow driver. A `GenStateMachine` per workflow instance that walks a
  `GtElixir.Workflow` module's declared steps for a specific bead,
  persisting progress to the DB on every transition so a crash + restart
  resumes at the same step.

  Pair with `GtElixir.Polecat` for orchestration: the polecat owns the
  ...
  *workflow's* step transitions. They collaborate but neither owns the
  ...
  implements the `GtElixir.Workflow` behaviour. Any module that does not
  ...
  alias GtElixir.Workflows.MachineRegistry
  alias GtElixir.Workflows.MachineState
  ...
  verify it implements `GtElixir.Workflow`.
  """
  ...
  defp implements_workflow_behaviour?(mod) do
    ...
    GtElixir.Workflow in behaviours
  end
