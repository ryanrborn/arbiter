defmodule GtElixir.Beads.Workspace.Changes.ValidateConfig do
  @moduledoc """
  ...
  Rules (loose — most keys are optional):
    ...
    * If `"tracker.type"` is present, it must be one of the values in
      `GtElixir.Beads.Workspace.valid_tracker_types/0` (`"none"`, `"jira"`,
      `"linear"`, `"github"`).
  ...
  (coordinator, worker, etc.) is deferred to `GtElixir.Vernacular` in gte-P2 —
  ...
  defp validate_tracker(changeset, tracker) when is_map(tracker) do
    valid_types = GtElixir.Beads.Workspace.valid_tracker_types()
