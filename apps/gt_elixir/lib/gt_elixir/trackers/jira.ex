defmodule GtElixir.Trackers.Jira do
  @moduledoc """
  Jira adapter implementing `GtElixir.Trackers.Tracker`.
  ...
  through `GtElixir.Trackers.Jira.Config`:
    ...
    2. `Application.get_env(:gt_elixir, :jira_default_config)` is the
       fallback for tools that run without a workspace context.
    3. With neither, callbacks return `{:error, %Error{kind: :config_missing}}`.

  This mirrors `GtElixir.Vernacular.put_active/1`. Two alternatives were
  ...
  *transition names* (not status names — Jira's REST API moves issues by
  ...
  `Application.get_env(:gt_elixir, :jira_http_stub, false)` is true, every
  request injects `plug: {Req.Test, #{inspect(GtElixir.Trackers.Jira.HTTP)}}`.
  This adapter **never** hits a real Atlassian endpoint from tests.
  """

  @behaviour GtElixir.Trackers.Tracker

  alias GtElixir.Trackers.Jira.{ADF, Config, Error}

  @stub_name GtElixir.Trackers.Jira.HTTP
  ...
  one-shot scripts.
  """
  @spec with_workspace(map() | GtElixir.Beads.Workspace.t(), (-> result)) :: result
        when result: any()
  ...
  defp stub_opts do
    if Application.get_env(:gt_elixir, :jira_http_stub, false) do
