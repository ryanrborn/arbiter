defmodule GtElixir.WorkflowTest do
  use ExUnit.Case, async: true

  alias GtElixir.Workflow
  alias GtElixir.Workflow.Example.GreetThenWave
  ...
  describe "behaviour callbacks on the example workflow" do
    ...
    test "the module declares the GtElixir.Workflow behaviour" do
  ...
  describe "Phase 5 composition placeholders" do
    ...
    test "composition options are stored on the module when provided" do
      mod_string = """
      defmodule GtElixir.WorkflowTest.PlaceholderShape do
  ...
  describe "run/2" do
    ...
    test "halts on {:error, reason} and reports {step, reason}" do
      defmodule BoomWorkflow do
        use GtElixir.Workflow, steps: [:ok_step, :bad_step]
    ...
    test "bad return from run_step is reported as :bad_return" do
      defmodule BadReturnWorkflow do
        use GtElixir.Workflow, steps: [:one]
    ...
    test "needs: violation halts before invoking run_step" do
      ...
      defmodule BadOrderWorkflow do
        use GtElixir.Workflow, steps: [:second_but_listed_first, :prereq]
  ...
  describe "compile-time validation" do
    test "step in `steps:` without a `step :name, ...` declaration raises CompileError" do
      bad = """
      defmodule GtElixir.WorkflowTest.MissingStepDecl do
        use GtElixir.Workflow, steps: [:a, :b]
    ...
    test "orphan `step :name, ...` not in `steps:` raises CompileError" do
      bad = """
      defmodule GtElixir.WorkflowTest.OrphanStep do
        use GtElixir.Workflow, steps: [:a]
    ...
    test "needs: referencing an unknown step raises CompileError" do
      bad = """
      defmodule GtElixir.WorkflowTest.UnknownNeed do
        use GtElixir.Workflow, steps: [:a, :b]
    ...
    test "non-list / non-atom-list `steps:` raises CompileError" do
      bad = """
      defmodule GtElixir.WorkflowTest.BadStepsOpt do
        use GtElixir.Workflow, steps: ["not", "atoms"]
      end
      """
