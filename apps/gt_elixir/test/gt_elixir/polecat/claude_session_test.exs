defmodule GtElixir.Polecat.ClaudeSessionTest do
  ...
  defp start_polecat(extra_opts \\ []) do
    ...
    {:ok, pid} =
      Polecat.start(Keyword.merge([bead_id: bead_id, rig: "gt-elixir"], extra_opts))
  ...
  describe "output streaming" do
    ...
    test "default topic is polecat:<bead_id> when :topic not provided" do
      ...
      :ok = Phoenix.PubSub.subscribe(GtElixir.PubSub, "polecat:#{bead_id}")
  ...
  describe "concurrent polecats" do
    test "each polecat sees only its own output" do
      ...
      :ok = Phoenix.PubSub.subscribe(GtElixir.PubSub, "polecat:#{bead_a}")
      :ok = Phoenix.PubSub.subscribe(GtElixir.PubSub, "polecat:#{bead_b}")
