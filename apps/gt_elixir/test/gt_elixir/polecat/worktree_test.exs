defmodule GtElixir.Polecat.WorktreeTest do
  ...
  setup do
    ...
    prior =
      case Application.fetch_env(:gt_elixir, @env_key) do
        {:ok, v} -> {:set, v}
        :error -> :unset
      end

    Application.put_env(:gt_elixir, @env_key, worktree_root)

    on_exit(fn ->
      case prior do
        {:set, v} -> Application.put_env(:gt_elixir, @env_key, v)
        :unset -> Application.delete_env(:gt_elixir, @env_key)
      end
