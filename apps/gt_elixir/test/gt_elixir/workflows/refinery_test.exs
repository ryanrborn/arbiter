defmodule GtElixir.Workflows.RefineryTest do
  ...
  alias GtElixir.Workflows.Refinery
  ...
  defp start_refinery(workspace, opts \\ []) do
    ...
    # test process for GtElixir.GitHub.HTTP.
    Req.Test.allow(GtElixir.GitHub.HTTP, self(), pid)
    # Allow it to use the Ecto sandbox connection too.
    Ecto.Adapters.SQL.Sandbox.allow(GtElixir.Repo, self(), pid)
    {pid, name}
  end
  ...
  describe "PubSub" do
    ...
    test "broadcasts {:bead_closed_by_refinery, bead_id} when merge lands", %{
      ...
      :ok = Phoenix.PubSub.subscribe(GtElixir.PubSub, "refinery:" <> ws.id)
