defmodule GtElixir.Workflows.MachineTest do
  ...
  describe "attach/3" do
    test "creates a MachineState row in :idle with current_step set to first step",
      ...
      assert row.workflow_module == "GtElixir.TestWorkflows.Three"
      assert row.vars == %{"x" => "hello"}
      assert row.state == %{"x" => "hello"}
      assert row.error_reason == nil
    end

    test "rejects a module that does not implement GtElixir.Workflow", %{issue: issue} do
      assert {:error, :not_a_workflow} =
               Machine.attach(GtElixir.TestWorkflows.NotAWorkflow, issue.id, %{})
    end

    test "rejects a non-existent module", %{issue: issue} do
      assert {:error, :unknown_module} =
               Machine.attach(GtElixir.NoSuch.Module.Definitely, issue.id, %{})
    end
  end
