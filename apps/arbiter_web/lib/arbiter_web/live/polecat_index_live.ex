defmodule ArbiterWeb.PolecatIndexLive do
  ...
  `Polecat.list_children/0` and is paged in memory. A status filter narrows to
  running vs awaiting work. Re-renders live on `:polecat_lifecycle` events and
  on a 1s tick (for the elapsed counters). Each row links to the polecat
  ...
  defp refresh(socket) do
    ...
    socket
    |> assign(:polecats, result.entries)
  ...
  defp polecat_path(:all, page), do: ~p"/polecats?#{%{page: page}}"
  defp polecat_path(status, page), do: ~p"/polecats?#{%{status: status, page: page}}"
    ...
    <Layouts.app flash={@flash} current_path={@current_path}>
      <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <.filter_tabs
          tabs={@filters}
          active={@status}
          tab_path={fn value -> polecat_path(value, 1) end}
        />

        <section class="card bg-base-200 border border-base-300 shadow-sm">
          <div class="card-body p-4 gap-4">
            <.empty_state :if={@polecats == []} id="polecats-empty" icon="hero-moon">
              No active {plural(@worker_label)} match this filter.
            </.empty_state>

            <ul :if={@polecats != []} id="polecats" class="flex flex-col gap-3">
              <li
                :for={p <- @polecats}
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  ...
                  <span class={["badge badge-sm shrink-0", polecat_status_class(p.status)]}>
            ...
            <.pager
              ...
              page_path={fn page -> polecat_path(@status, page) end}
            />
          </div>
        </section>
