defmodule ArbiterWeb.BeadIndexLive do
  @moduledoc """
  Index of every directive (bead) at `/beads` — the "See all" target for the
  ...
  `:bead_lifecycle` events so a transition shows up without a refresh.
  """
  ...
  @impl true
  # Any bead transition can change which rows belong on the current page;
  ...
  defp bead_path(:all, page), do: ~p"/beads?#{%{page: page}}"
  defp bead_path(status, page), do: ~p"/beads?#{%{status: status, page: page}}"
    ...
    <Layouts.app flash={@flash} current_path={@current_path}>
      <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <.filter_tabs
          tabs={@filters}
          active={@status}
          tab_path={fn value -> bead_path(value, 1) end}
        />

        <section class="card bg-base-200 border border-base-300 shadow-sm">
          <div class="card-body p-4 gap-4">
            <.empty_state :if={@beads == []} id="beads-empty" icon="hero-clipboard-document-list">
              No {plural(@issue_label)} match this filter.
            </.empty_state>

            <ul :if={@beads != []} id="beads" class="flex flex-col gap-1.5">
              <li
                :for={b <- @beads}
                class={[
            ...
            <.pager
              ...
              page_path={fn page -> bead_path(@status, page) end}
            />
          </div>
        </section>
