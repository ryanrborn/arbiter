defmodule ArbiterWeb.WorkspaceDetailLive do
    ...
    case Ash.get(Workspace, id) do
      {:ok, ws} ->
        {:ok,
         ...
         |> assign(:tracker_config_type_preview, cfg(ws, ["tracker", "type"], "none"))
  ...
  def handle_event("preview_tracker_type", %{"config" => params}, socket) do
    {:noreply, assign(socket, :tracker_config_type_preview, params["tracker_type"] || "none")}
  end
  ...
  # ---- tracker.config.* (adapter-specific fields, scoped to tracker.type) ----
    ...
    case patch_config(socket.assigns.workspace, patch, unset) do
      {:ok, ws} ->
        {:noreply,
         ...
         |> assign(:tracker_config_error, nil)
         |> load_derived()
         |> put_flash(:info, "Tracker config saved.")}
  ...
  # ---- tracker.config.* form prefill / patch ----
      ...
      <div class="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
        ...
        <section class="card bg-base-200 border border-base-300 shadow-sm">
          <div class="card-body p-4 gap-3">
            ...
            <div :if={@tracker_config_type_preview != "none"} class="border-t border-base-300 pt-3">
              <h3 class="font-semibold text-sm flex items-center gap-2">
                <.icon name="hero-ticket" class="size-4 text-base-content/60" />
                {String.capitalize(@tracker_config_type_preview)} tracker config
              </h3>
              <p class="text-xs text-base-content/50 mt-1">
                <code>tracker.config.*</code>
                — fields the {@tracker_config_type_preview} adapter reads. Switching the tracker
              ...
              >
                <input type="hidden" name="tracker_config[type]" value={@tracker_config_type_preview} />
                <.input
                  :for={{field, label} <- tracker_adapter_fields(@tracker_config_type_preview)}
                  type="text"
