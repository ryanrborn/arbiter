defmodule GtElixirCli.Cmd.Doctor do
  @moduledoc """
  `bd2 doctor` — health checks.

  Currently runs:

    1. Can we reach `GET /api/workspaces`? (Phoenix reachable)
    2. Does at least one workspace exist? (DB reachable + reasonable state)
    3. Can we resolve the configured workspace? (BD2_WORKSPACE / "default")
  ...
  defp emit_text(results) do
    IO.puts("bd2 doctor — checks against #{Client.base_url()}")
    IO.puts("")
