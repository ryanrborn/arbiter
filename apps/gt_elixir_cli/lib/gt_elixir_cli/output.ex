defmodule GtElixirCli.Output do
  @moduledoc """
  ...
  @doc """
  One-line summary used by `bd2 list` and `bd2 ready`. Format:
  ...
  @doc """
  Multi-section detail view used by `bd2 show`. Sections (only emitted when
  ...
  def die(msg) when is_binary(msg) do
    IO.puts(:stderr, "bd2: error: " <> msg)
    do_halt(1)
  end

  def die(%Client.Error{} = err) do
    IO.puts(:stderr, "bd2: error: " <> err.message)
  ...
  defp do_halt(code) do
    case Process.get(:bd2_halt_strategy, :system_halt) do
      :raise -> raise GtElixirCli.Output.Halt, code: code
